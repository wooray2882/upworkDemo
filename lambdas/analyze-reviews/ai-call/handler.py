...
Supports two input modes:
  ...
  - s3_key: an uploaded file (see modules/core-engine/file-uploads.tf) - a
    .xlsx review export is parsed directly into rows; a PDF/image (e.g. an
    exported review report) is read directly by Claude's document/vision
    support, same as extract-document - no Textract involved.
"""
import base64
import os
...
def lambda_handler(event, context):
    ...
    if s3_key:
        ...
        if s3_key.lower().endswith(".xlsx"):
            reviews_text = parse_xlsx_rows(file_bytes)
